...
class PIE_Form_Task {
	...
	public function listen_task() {
		if ( ! empty( $_GET['pie_forms_return'] ) ) { 
			$this->entry_confirmation_redirect( '', wp_unslash( $_GET['pie_forms_return'] ) ); 
		}
		
		if ( ! empty( $_POST['pie_forms']['id'] ) ) { 
			$this->do_task( stripslashes_deep( $_POST['pie_forms'] ) ); 
		}
	}
	...
	public function do_task( $entry ) {
		try {
			...
			$this->form_data = apply_filters( 'pie_forms_process_before_form_data', Pie_Forms()->core()->pf_decode( wp_unslash($form->form_data) ), $entry );
			...
			foreach ( $this->form_data['form_fields'] as $field ) {
				...
				if ( 'gdpr' === $field_type && $field_submit === "on" ) {
					$this->gdpr = true;
				}
			...
			if ( ! empty( $site_key ) && ! empty( $secret_key ) && isset( $this->form_data['settings']['recaptcha_support'] ) && '1' === $this->form_data['settings']['recaptcha_support'] ) {
				...
				$token = ! empty( $_POST['g-recaptcha-response'] ) ? wp_unslash( $_POST['g-recaptcha-response'] ) : false;

				if ( 'v3' === $recaptcha_type ) {
					$token = ! empty( $_POST['pie_forms']['recaptcha'] ) ? wp_unslash( $_POST['pie_forms']['recaptcha'] ) : false;
				}
			...
			if ( ! empty( $errors[ $form_id ] ) ) {
				if ( empty( $errors[ $form_id ]['header'] ) ) {
					$errors[ $form_id ]['header'] = __( 'Form has not been submitted, please see the errors below. pie initial', 'pie-forms' );
				}
				$this->errors = $errors;
				return $this->errors;
			}
			...
			if ( $honeypot ) {
				echo "error found of honepot from id ". absint( $this->form_data['id'] );
				return $this->errors;
			}
			...
			//is checked on gdpr from front end form.
			if($this->gdpr){
		...
		if ( '1' === $ajax_form_submission ) {
			...
			add_filter('pie_form_success_message',array( $this, 'pf_print_message' ), 10, 2);
	...
	public function pf_print_message(){
