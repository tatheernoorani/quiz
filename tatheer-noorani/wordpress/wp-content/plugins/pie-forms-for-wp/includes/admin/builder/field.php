...
if ( class_exists( 'PIE_Admin_Builder_Field', false ) ) {
	return new PIE_Admin_Builder_Field();
}

/**
 * PIE_Admin_Builder_Field class.
 */
class PIE_Admin_Builder_Field extends PIE_Admin_Builder_Page {
	...
	public function output_fields() {
        ...
        if ( ! empty( $form_fields ) ) {
          ...
          foreach ( $form_fields as $group => $form_field ) {
          	$active = ($i === 0) ? "active" : "";
            ?>
                <li class="<?php echo $active; ?>"><span> <?php echo Pie_Forms()->core()->get_fields_group($group); ?> </span>
                  <ul class="pie-form-fields" id="pie-form-draggable" >
                  <?php foreach ( $form_field as $field ) : ?>
                      <li class="pie-form-element-box <?php echo sanitize_html_class( $field->class ); ?> <?php echo $field->type; ?>" data-field-type="<?php echo $field->type?>">
                        <span class="pie-form-icon">
                            <img src="<?php echo Pie_Forms::$url . 'assets/images/builder/'.$field->icon . '.png' ?>" alt="">
                          </span>
                        <span class="pie-form-label"><?php echo $field->name; ?></span>
                      </li>
                      <?php endforeach; ?>
                  </ul> 
                </li>
	...
	//	$is_pro = pie_forms()->core()->get_pro_form_field_types();
		...
		printf( '<div class="pie-forms-field dropper pie-forms-field-%1$s %2$s" id="pie-forms-field-%3$s" data-field-id="%3$s" data-field-type="%4$s">', esc_attr( $field['type'] ), esc_attr( $css ), esc_attr( $field['id'] ), esc_attr( $field['type'] ) );
			...
			do_action( 'pie_forms_builder_fields_preview_' . $field['type'], $field );
