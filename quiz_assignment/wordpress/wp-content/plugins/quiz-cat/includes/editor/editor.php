		...
		$html = '<div id="qc-nav">';
			...
			$html .= '<h1 class="nav-tab-wrapper">';
				...
				if ( function_exists ('fca_qc_save_appearance_settings') ) {
					$html .= '<a href="#" id="appearance-nav" class="nav-tab">' . __('Appearance', 'quiz-cat') . '</a>';
				}
			$html .= '</h1>';
...
function fca_qc_render_answer( $answer, $question_number, $answer_number, $correct_answer ) {
	...
	$html .= "<div class='fca_qc_answer_input_div fca_qc_deletable_item'>";
		...
		if ( $answer_number == $correct_answer ) {
			$html .= "<p class='fca_qc_answer_header'>" . __('Correct Answer', 'quiz-cat') . "</p>";
		} else {
			$html .= "<p class='fca_qc_answer_header'>" . __('Wrong Answer', 'quiz-cat') . fca_qc_add_delete_button() . '</p>';
		}
		$html .= "<table class='fca_qc_inner_setting_table'>";
	...
	<div id='fca-quiz-select'>
		<table id='fca-quiz-select-wrapper'>
			<tr>
				...
				<h2> <?php echo __( 'Choose your quiz type', 'quiz-cat' ) ?></h2>
			</tr>

			<tr>
				<td class="fca-quiz-select-option">
					<p class='fca_qc_type_header'> <?php echo __( 'Multiple Choice', 'quiz-cat' )?></p><br>
					<p> <?php echo __( "Test a person's knowledge.", 'quiz-cat' ) ?></br>
					<?php echo __( "Each question has one correct answer.", 'quiz-cat' ) ?></p>
					<p class='fca_qc_info_span'> <?php echo __( "e.g.:", 'quiz-cat' ) ?> <br>
					<?php echo __( "How much do you know about Star Wars?", 'quiz-cat' ) ?><br>
					<?php echo __( "Can you pass a U.S. citizenship test?", 'quiz-cat' ) ?></p>
					<div class='fca_qc_quiz_select_button'>
						<button class='button-primary button' id='fca-quiz-select-multiplechoice' type='button'> <?php echo __( "Build a Multiple Choice Quiz", 'quiz-cat' ) ?></button>
					</div>
				</td>

				<td class="fca-quiz-select-option <?php echo $free ?>">
					<p class='fca_qc_type_header'><?php echo __( 'Personality Type', 'quiz-cat' )?></p><br>
					<p> <?php echo __( "Determine a person's personality or preference.", 'quiz-cat' ) ?></br>
					<?php echo __( "There are no right or wrong answers.", 'quiz-cat' ) ?></p>
					<p class='fca_qc_info_span'> <?php echo __( "e.g.:", 'quiz-cat' ) ?> <br>
					<?php echo __( "Which Star Wars character are you?", 'quiz-cat' ) ?><br>
					<?php echo __( "Which WordPress host is right for you?", 'quiz-cat' ) ?></p>
					<div class='fca_qc_quiz_select_button'>
						<?php if ( $free ){ ?>
							<a href='https://fatcatapps.com/quizcat/pricing/' target='_blank' class='not-allowed button fca_quiz_premium_redirect' type='button'> <?php echo __( 'Get Quiz Cat Premium', 'quiz-cat') ?></a>
						<?php } else { ?>
							<button class='button-primary button' id='fca-quiz-select-personality' type='button'> <?php echo __( "Build a Personality Type Quiz", 'quiz-cat' ) ?></button>
						<?php } ?>
					</div>
				</td>
				<?php if ( function_exists( 'fca_qc_add_weighted_metaboxes' ) || $free ) { ?>
					<td class="fca-quiz-select-option <?php echo $free ?>">
						<p class='fca_qc_type_header'><?php echo __( 'Weighted Answers', 'quiz-cat' )?></p><br>
						<p> <?php echo __( "Assign points to each answer.", 'quiz-cat' ) ?></br>
						<?php echo __( "The more points someone scores, the better their result.", 'quiz-cat' ) ?></p>
						<p class='fca_qc_info_span'> <?php echo __( "e.g.:", 'quiz-cat' ) ?> <br>
						<?php echo __( "How healthy is your diet?", 'quiz-cat' ) ?><br>
						<?php echo __( "How adventurous are you?", 'quiz-cat' ) ?></p>
						<div class='fca_qc_quiz_select_button'>
							<?php if ( $free ){ ?>
								<a href='https://fatcatapps.com/quizcat/pricing/' target='_blank' class='not-allowed button fca_quiz_premium_redirect' type='button'> <?php echo __( 'Get Quiz Cat Premium', 'quiz-cat') ?></a>
							<?php } else { ?>
								<button class='button-primary button' id='fca-quiz-select-weighted' type='button'> <?php echo __( "Build a Weighted Answers Quiz", 'quiz-cat' ) ?></button>
							<?php } ?>
						</div>
					</td>
				<?php } ?>
...
function fca_qc_render_quiz_settings_meta_box( $post ) {
	...
	echo "<table class='fca_qc_setting_table'>";
		...
		if ( function_exists ('fca_qc_save_quiz_settings_premium' ) ) {
			...
			echo "<tr>";
				echo "<th>";
					echo "<label class='fca_qc_admin_label fca_qc_admin_settings_label' for='fca_qc_fixed_answer_order'>" . __('Fixed Answer Order', 'quiz-cat') . fca_qc_tooltip(__( 'Best used for True/False or Yes/No quizzes. If enabled, manually drag and drop the correct answer into the desired position in the Editor tab.','quiz-cat')) . "</label>";
				echo "</th>";
				echo "<td>";
					echo "<div class='onoffswitch'>";
