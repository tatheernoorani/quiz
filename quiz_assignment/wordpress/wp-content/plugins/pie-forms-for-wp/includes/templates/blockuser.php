	...
	<form method="<?php echo esc_attr( apply_filters( 'pie_forms_settings_form_method_tab_' . $current_tab, 'post' ) ); ?>" id="mainform" action="" enctype="multipart/form-data">
		<nav class="nav-tab-wrapper pf-nav-tab-wrapper">
			...
			foreach ( $tabs as $slug => $label ) {
				echo '<a href="' . esc_html( admin_url( 'admin.php?page=pf-blockuser&tab=' . esc_attr( $slug ) ) ) . '" class="nav-tab ' . ( $current_tab === $slug ? 'nav-tab-active' : '' ) .' tab_'.$slug.'"><span class="pf-nav-icon ' . esc_attr( $slug ) . '"></span>' . esc_html( $label ) . '</a>';
			}
		...
		<?php 
			?>
			<div class="pie-forms-blockuser" id='<?php echo $current_tab; ?>'>
			...
			<h1 class="blockuser-page-heading"><?php echo esc_html( $current_tab_label ); ?></h1>
				...
				?>	
				<div class="upgrade-to-pro-banner"><a href="https://pieforms.com/plan-and-pricing/?utm_source=admindashboard&utm_medium=adminfeatures&utm_campaign=blockusers" target="_blank"><img src="<?php echo plugins_url( 'assets/images/builder/upgrade-to-premium.jpg', dirname(__DIR__) )  ?>" alt="upgrade-to-premium"></a></div>
			</div>
