...
class PIE_Admin_AllFormTable extends WP_List_Table
{
    /** Class constructor */
    ...
    public function usort_reorder( $a, $b ) {
        ...
        $orderby = ( ! empty( $_GET['orderby'] ) ) ? $_GET['orderby'] : 'form_name';
    ...
    function prepare_items() {
        ...
        $status = (isset($_REQUEST['status']) && $_REQUEST['status']) ? $_REQUEST['status'] : 'published'; 
        ...
        $user_search_key = isset( $_POST['s'] ) ? wp_unslash( trim( $_POST['s'] ) ) : '';
    ...
    function column_form_name($item) {
		...
		$output .= '</strong>';
            ...
            if(isset($_REQUEST['status']) && $_REQUEST['status'] == "trash"){
                ...
                $actions['delete'] = sprintf('<a href="?page=%s&amp;action=%s&amp;form=%s&amp;_wpnonce=%s">Delete Permanently</a>', esc_attr( $_REQUEST['page'] ),'delete',absint( $item['id'] ), $delete_nonce);
