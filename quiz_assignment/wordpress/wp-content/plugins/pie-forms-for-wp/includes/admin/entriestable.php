 ...
 */
class PIE_Admin_EntriesTable extends WP_List_Table {
	...
	public function __construct() {
		// Fetch all forms.
		$this->forms = Pie_Forms()->core()->pf_get_all_forms( );
		add_action( 'admin_init', array( $this, 'actions' ) );

		// Check that the user has created at least one form.
		if ( ! empty( $this->forms ) ) {
			$this->form_id   = ! empty( $_REQUEST['form_id'] ) ? absint( $_REQUEST['form_id'] ) : apply_filters( 'pie_forms_entry_list_default_form_id', key( $this->forms ) ); 
	...
	public function column_form_field( $entry, $column_name ) {
		...
		if ( ! empty( $entry->meta[ $meta_key ] ) ) { 
			...
			return apply_filters( 'pie_forms_html_field_value', $value, $entry->meta[ $meta_key ], $entry, 'entry-table' );
		} else {
	...
	private function get_status_label( $status_name, $amount ) {
		$statuses = $this->pf_get_entry_statuses( $this->form_data );
	...
	protected function get_views() {
		$status_links  = array();
		$num_entries   = $this->pf_get_count_entries_by_status( $this->form_id );
		$total_entries = apply_filters( 'pie_forms_total_entries_count', (int) $num_entries['publish'], $num_entries, $this->form_id );
		$statuses      = array_keys( $this->pf_get_entry_statuses( $this->form_data ) );
     ...
     */
    function pf_get_count_entries_by_status( $form_id ) {
        $form_data = Pie_forms()->form()->get( $form_id, array( 'content_only' => true ) );
        $statuses  = array_keys( $this->pf_get_entry_statuses( $form_data ) );
		$counts    = array();
		
        foreach ( $statuses as $status ) {
            $count = count(
                $this->pf_search_entries(
                    array(
                        'limit'   => -1,
                        'status'  => $status,
                        'form_id' => $form_id,
     ...
     * Get entry statuses.
     */
    function pf_get_entry_statuses( $form_data = array() ) {
	...
	public function forms_dropdown() {
		$forms   = Pie_Forms()->core()->pf_get_all_forms( );
	...
	public function prepare_items() {
        ...
        $entries     = $this->pf_search_entries( $args );
        $columns = $this->get_columns();
        $hidden = array();
        $sortable = $this->get_sortable_columns();
        $this->_column_headers = array($columns, $hidden, $sortable);
		$PIE_Database_Models_FormsEntries = new PIE_Database_Models_FormsEntries();
        $this->items = array_map( array($PIE_Database_Models_FormsEntries, 'pf_get_entry'), $entries );
        $this->views();
		// Get total items.
		$args['limit']  = -1;
		$args['offset'] = 0;
		$total_items    = count( $this->pf_search_entries( $args ) );
     ...
     * Search entries.
     */
    function pf_search_entries( $args ) {
        global $wpdb;
        $args = wp_parse_args(
            $args,
            array(
        ...
        if ( ! array_key_exists( $args['form_id'], Pie_Forms()->core()->pf_get_all_forms() ) ) {
            return array();
        }
