...
class PIE_Shortcodes_Block {
	...
	public function get_form_html( $attr ) {
		...
		return PIE_Shortcodes_Display::shortcode_wrapper(
			array( 'PIE_Shortcodes_Form', 'output' ),
			array(
				'id'          => $form_id,
				'title'       => $title,
			...
			array(
				'class' => PIE_Forms()->core()->pf_sanitize_classes( $classes ),
			)
		);
	}
}