...
class PIE_Email_Settings {
	...
	public function build_email( $message ) {
		...
		Pie_Forms()->core()->pf_get_template( 'email/header-default.php' );

		// Hooks into the email header.
		do_action( 'pie_forms_email_header', $this );

		Pie_Forms()->core()->pf_get_template( 'email/body-default.php' );

		// Hooks into the email body.
		do_action( 'pie_forms_email_body', $this );

		Pie_Forms()->core()->pf_get_template( 'email/footer-default.php' );
	...
	public function send( $to, $subject, $message, $attachments = '' ) {
		...
		$subject           = Pie_Forms()->core()->pf_decode_string( $this->process_tag( $subject ) );
	...
	public function process_tag( $string = '', $sanitize = true, $linebreaks = false ) {
		...
		//$tag = pf_decode_string( $tag );

		if ( $sanitize ) {
			if ( $linebreaks ) {
				$tag = pf_sanitize_textarea_field( $tag );
			} else {
	...
	public function pie_forms_html_field_value( $html = true ) {
		...
		if ( $html ) {
			...
			Pie_Forms()->core()->pf_get_template( 'email/field-default.php' );
			...
			foreach ( $this->fields as $meta_id => $field ) {
				...
				$field_value = apply_filters( 'pie_forms_html_field_value', pf_decode_string( $field_val ), $field['value'], $this->form_data, 'email-html' );
			...
			foreach ( $this->fields as $field ) {
				...
				$message    .= '--- ' . pf_decode_string( $field_name ) . " ---\r\n\r\n";
				$field_value = pf_decode_string( $field_val ) . "\r\n\r\n";
	...
	public function get_template() {
		if ( ! $this->template ) {
			$this->template = get_option( 'pie_forms_email_template', 'default' );
		}
