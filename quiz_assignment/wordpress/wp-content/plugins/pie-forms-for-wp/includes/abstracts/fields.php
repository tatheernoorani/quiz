 ...
 * Form fields class.
 */
abstract class PIE_Abstracts_Fields {
	...
	public function __construct() {
		...
		add_action( 'pie_forms_builder_fields_preview_' . $this->type, array( $this, 'field_preview' ) );
	...
	public function field_option( $option, $field, $args = array(), $echo = true ) {
		...
		switch ( $option ) {
			 ...
			 */
			case 'meta':
				$value   = ! empty( $field['meta-key'] ) ? esc_attr( $field['meta-key'] ) : PIE_Forms()->core()->pf_get_meta_key_field_option( $field );
			...
			case 'choices':
				...
				$input_type = in_array( $field['type'], array( 'radio' ), true ) ? 'radio' : 'checkbox';
				...
				$field_content = sprintf(
					...
					PIE_Forms()->core()->pf_sanitize_classes( $class, true ),
					$field['id'],
					$this->type
				);
	...
	public function field_preview_option( $option, $field, $args = array(), $echo = true ) {
		$output = '';
		$class  = ! empty( $args['class'] ) ? PIE_Forms()->core()->pf_sanitize_classes( $args['class'] ) : '';
		...
		switch ( $option ) {
			...
			case 'choices':
				...
				$choices_fields = array( 'select', 'radio', 'checkbox', 'multiselect' );
				...
				if ( 'select' === $type || 'multiselect' === $type ) {
					...
					$output      = sprintf( '<select class="%s" disabled>', PIE_Forms()->core()->pf_sanitize_classes( $list_class, true ) );
					...
					$output = sprintf( '<ul class="%s">', PIE_Forms()->core()->pf_sanitize_classes( $list_class, true ) );
					...
					foreach ( $values as $value ) {
						...
						$output .= sprintf( '<li class="%s">', PIE_Forms()->core()->pf_sanitize_classes( $item_class, true ) );
	...
	public function field_new() {
		...
		// Grab field data.
		$field_args     = ! empty( $_POST['defaults'] ) ? (array) wp_unslash( $_POST['defaults'] ) : array(); 
		$field_type     = esc_attr( wp_unslash( $_POST['field_type'] ) ); 
		$field_id       = PIE_Forms()->core()->field_unique_key( wp_unslash( $_POST['form_id'] ) ); 
		$field          = array(
			'id'          => $field_id,
			'type'        => $field_type,
			'label'       => $this->name,
	...
	public function validate( $field_id, $field_submit, $form_data ) {
		...
		$validation_rule = isset($form_data['form_fields'][$field_id]['validation_rule']) ? $form_data['form_fields'][$field_id]['validation_rule']: "";
		
		$custom_validation_message = isset($form_data['form_fields'][$field_id]['custom_validation_message']) ? $form_data['form_fields'][$field_id]['custom_validation_message']: "Please enter a valid value";
		...
		if ( false !== $required_field && '1' !== $conditional_status && ( empty( $field_submit ) && '0' !== $field_submit ) ) {
			...
			Pie_Forms()->task->errors[ $form_data['id'] ][ $field_id ] = Pie_Forms()->core()->pf_get_required_label();
		}
		
		if(!empty($validation_rule)){
			if(Pie_Forms()->core()->get_define_regex($_POST['pie_forms']['form_fields'][ $field_id ],$custom_regex))
				{
		...
		switch ( $field_type ) {
			case 'phone':
				$custom_regex = "phone";
				if ( !empty( $_POST['pie_forms']['form_fields'][ $field_id ] ) && Pie_Forms()->core()->get_define_regex($_POST['pie_forms']['form_fields'][ $field_id ],$custom_regex) ) { 
		...
		if ( isset( $validation_text ) ) {
			Pie_Forms()->task->errors[ $form_data['id'] ][ $field_id ] = apply_filters( 'pie_forms_type_validation', $validation_text );
	...
	public function format( $field_id, $field_submit, $form_data, $meta_key ) {
		...
		$value = Pie_Forms()->core()->pf_sanitize_textarea_field( $field_submit );
